...
console.log(
  [
    ...
    '',
    '--- If Forbidden on `deploy --prod` (createSiteDeploy) — use a BUILD HOOK instead ---',
    'No Netlify account permission needed beyond creating the hook once in the UI.',
    '1) Push your commits:  git push origin main',
    '2) Netlify → Build & deploy → Build hooks → Add (branch main) → copy URL',
    '3)  export NETLIFY_BUILD_HOOK_URL=\'https://api.netlify.com/build_hooks/...\'',
    '    npm run trigger:build',
    'Or: npx netlify deploy --trigger   (uses createSiteBuild, not createSiteDeploy)',
    '',
